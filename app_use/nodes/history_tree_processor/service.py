...
from app_use.nodes.history_tree_processor.view import AppHistoryElement, HashedAppElement
...
class HistoryTreeProcessor:  # noqa: D101 – public API mirrors web implementation
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def convert_app_element_to_history_element(app_element: AppElementNode) -> AppHistoryElement:  # noqa: D401
        ...
        str_props: Dict[str, str] = {k: str(v) for k, v in app_element.properties.items()}

        return AppHistoryElement(
            node_type=app_element.node_type,
            node_path=node_path,
    ...
    def compare_history_element_and_app_element(
        ...
        return HistoryTreeProcessor._hash_app_history_element(
            app_history_element
        ) == HistoryTreeProcessor._hash_app_element(app_element)

    # ------------------------------------------------------------------
    #   Hash helpers (private)
    # ------------------------------------------------------------------
    @staticmethod
    def _hash_app_history_element(app_history_element: AppHistoryElement) -> HashedAppElement:
        branch_path_hash = HistoryTreeProcessor._parent_branch_path_hash(
            app_history_element.entire_parent_branch_path
        )
    ...
    def _get_parent_branch_path(app_element: AppElementNode) -> List[str]:
        ...
        parents.reverse()
        return [p.node_type for p in parents]
